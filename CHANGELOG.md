...
are not minor changes — they rename values, change what a field means and
remove one. §17 now says plainly that a pre-1.0 minor promises no
compatibility, because promising it and breaking it is worse than not promising
it: a reader trusting minor compatibility opens a 0.3 container with 0.2
assumptions and gets a plausible wrong answer.

**What breaks a reader written for 0.2**

- **Extension names.** `ash_clinical`, `ash_gs_measured` and `ash_reversible`
  are `histora_clinical`, `histora_gs_measured` and `histora_reversible`.
  Renamed, not aliased.
- **Profile identifiers.** `histora-gs-apariencia/1.0` is
  `histora-gs-appearance/1.0`; `histora-twin-ajustado/1.0` is
  `histora-twin-fitted/1.0`.
- **The composite PLY column `origen`** is `source_modality`.
- **Scene node names and `.gs.json` `role` values** are English:
  `"campo gaussiano del twin"` is `"twin density field"`, and the other three
  likewise.
...
- **The FDI code no longer travels in any layer-1 asset**, in any of its three
  ...
  is stripped. The mesh ships as a single primitive.
- **`digest_of_parts` is computed over SOP Instance UID and pixel hash** when
  every part declares them, falling back to name and file hash otherwise. A
  0.2 container still verifies; a 0.3 one identifies the clinical series rather
  than one serialisation of it.
- **`Regulatory` is `{layer, clearances}`.** `status` and `jurisdictions` are
  gone: a status without a jurisdiction was not a statement about anything.
- **The conformance fixture is renamed**, cases and index alike:
  `fixtures/uos-0.3/expected.json`, keys `file`/`expects`/`why`, classes
  `valid`/`error`/`warning`/`valid-with-warning`.

**Added**

- `Locator` and `Asset.locators`: where to find a referenced original, as a
  hint. `identifying` has no default, and check 17o makes an identifying
  locator an error in a de-identified container.
- Stable finding codes (`UOS-E-005`, `UOS-E-017b`) and
  `schemas/uos-validation-report-0.3.schema.json`.
- Field documentation on every type, so the published schema documents all 103
  fields instead of none, and the specification's field tables are generated
  from those same types.
- `uos.marcos`: what traversing the registration graph means — inversion,
  composition order, deterministic path choice, and check 15b for paths that
  disagree.
- De-identification, purpose of use, occlusion, per-value regulatory layers,
  DICOM identity on parts and frames, and the `UOS-Distributable` profile.
- A whole manifest in Appendix B, lifted from the fixture.



### Added

- **UOS format specification** (`docs/spec/uos-format-spec-v0.3.tex`): a self-contained
...
- **The published JSON Schema is entirely in English.** Field names and enum values already
  ...
  Spanish class names and docstrings, so `schemas/uos-manifest-0.3.schema.json` shipped
