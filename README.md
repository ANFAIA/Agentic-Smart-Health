...
The measured event log behind the SVG is versioned at
[`docs/assets/pipeline-demo-events.json`](docs/assets/pipeline-demo-events.json). The
...
| Script | What it does |
|---|---|
| [`scripts/ablacion_recetas.py`](scripts/ablacion_recetas.py) | Training recipe ablation: what each component contributes. |
| [`scripts/altura_corona.py`](scripts/altura_corona.py) | Measures clinical crown height on the intraoral scanner mesh. |
| [`scripts/audit_pr.py`](scripts/audit_pr.py) | Architecture guardian for pull requests in the monorepo. |
| [`scripts/blender_render_views.py`](scripts/blender_render_views.py) | Headless Blender multi-view renderer for intraoral meshes. |
| [`scripts/caso_completo.py`](scripts/caso_completo.py) | Runs the full pipeline over one real clinical case, stage by stage. |
| [`scripts/composicion_cbct_ios.py`](scripts/composicion_cbct_ios.py) | Composes CBCT teeth and IOS gum geometry as Gaussians. |
| [`scripts/data_guard.py`](scripts/data_guard.py) | Blocks third-party, oversized or clinical data from entering the repository. |
| [`scripts/demo_pipeline_readme.py`](scripts/demo_pipeline_readme.py) | Regenerates the README pipeline animation from the synthetic case. |
| [`scripts/desplazamiento_relativo.py`](scripts/desplazamiento_relativo.py) | Measures whether tooth displacement can be reported robustly. |
| [`scripts/docs_sync.py`](scripts/docs_sync.py) | Checks that documentation stays aligned with code and repository inventory. |
| [`scripts/entrena_diente_cbct.py`](scripts/entrena_diente_cbct.py) | Trains the CBCT tooth segmenter against the threshold baseline. |
| [`scripts/entrena_gs_escaner.py`](scripts/entrena_gs_escaner.py) | Trains real 3DGS over the scanner surface. |
| [`scripts/entrenar_3dgs.py`](scripts/entrenar_3dgs.py) | Negative experiment: trains 3DGS from a dental arch. |
| [`scripts/eval_informes.py`](scripts/eval_informes.py) | Measures how much report content reaches the shared data contract. |
| [`scripts/fetch_knowledge_base.py`](scripts/fetch_knowledge_base.py) | Materialises the research agent knowledge base. |
| [`scripts/fetch_teeth3ds.sh`](scripts/fetch_teeth3ds.sh) | Downloads Teeth3DS+ reproducibly from the official Google Drive source. |
| [`scripts/genera_fixture.py`](scripts/genera_fixture.py) | Generates the format's conformance fixture: one valid container and several broken. |
| [`scripts/malla_mejorada.py`](scripts/malla_mejorada.py) | Builds the improved STL from the UOS container and nothing else. |
| [`scripts/metricas.py`](scripts/metricas.py) | Computes the measured metrics promised in the project brief. |
| [`scripts/mide_segmentacion.py`](scripts/mide_segmentacion.py) | Measures how much FDI segmentation can be discarded from a `.uos` file. |
| [`scripts/promedio_y_escala.py`](scripts/promedio_y_escala.py) | Measures design questions around per-tooth registration and scaling. |
| [`scripts/refina_3dgs.py`](scripts/refina_3dgs.py) | Optimises the seeded Gaussian field as a 3DGS representation. |
| [`scripts/registro_ios_cbct.py`](scripts/registro_ios_cbct.py) | Measures whether the intraoral scanner and CBCT can be aligned. |
| [`scripts/resolucion_modalidades.py`](scripts/resolucion_modalidades.py) | Simulates the effective resolution of each dental modality. |
| [`scripts/segmentar_fdi.py`](scripts/segmentar_fdi.py) | Labels each tooth in a dental arch with its FDI code. |
| [`scripts/seguimiento_histora.py`](scripts/seguimiento_histora.py) | Measures gingival margin movement between two scans. |
| [`scripts/verifica_contenedor.py`](scripts/verifica_contenedor.py) | Verifies that a `.uos` container tells the truth about itself. |
| [`scripts/watch_literature.py`](scripts/watch_literature.py) | Watches scientific literature and proposes manifest entries. |
| [`scripts/white_paper_docx.py`](scripts/white_paper_docx.py) | Converts the white paper's LaTeX source to .docx for reviewers to edit. |
<!-- /generado: scripts -->
