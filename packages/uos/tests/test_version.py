...
de las versiones menores —«un lector v0.3 abre un contenedor v0.4»— no la podia cumplir
...
def test_una_menor_superior_se_lee_IGNORANDO_lo_que_no_se_conoce():
    ...
    crudo = _manifiesto_crudo(uos_version="0.4", campo_de_la_04="lo que sea")

    m, ignorados = read_manifest_from(crudo)

    assert m.case_id == "urn:uuid:0"
    assert ignorados == ["campo_de_la_04"], "y se dice CUAL se ignoro, no se calla"
...
def test_se_ignora_tambien_dentro_de_un_asset_anidado():
    ...
    m, ignorados = read_manifest_from(_manifiesto_crudo(uos_version="0.4", assets=[asset]))
...
def test_leer_permisivo_y_REEMITIR_no_se_combinan():
    ...
    Un lector v0.3 que abre una v0.4, ignora lo que no entiende y escribe la version N+1
    ...
    """
    assert puede_reemitir("0.3") and puede_reemitir("0.2")
    assert not puede_reemitir("0.4"), "leerla si, sucederla no"
...
@pytest.mark.parametrize(("version", "rama"), [
    ...
    ("0.3", Lectura.ESTRICTA),
    ("0.4", Lectura.PERMISIVA), ("0.9", Lectura.PERMISIVA),
