...
\code{encoding.count}, and where the sidecar declares
\code{source\_positions\_sha256} it matches the source asset's \code{POSITION} accessor.\\
...
requires, and the algorithm has to say so. A container declaring a higher \textbf{minor} is
parsed leniently, ignoring and naming the fields the reader does not know --- and running
check 4 first would reject it as malformed, which contradicts the compatibility a minor is
defined to promise.
