...
\midrule
\endhead
No FDI in the scene & A primitive of an asset whose \code{regulatory.layer} is not 3
\MUSTNOT{} declare \code{extras.uos\_fdi}, and \MUSTNOT{} declare the
\code{\_REGION\_ID} attribute on a \code{KHR\_gaussian\_splatting} primitive. Both are FDI
codes and both come from a segmentation model.\\
...
17b & E & Layer content in the scene (\S\ref{sec:picking}): for every asset whose
\code{regulatory.layer} is not 3 and whose \code{uri} ends in \code{.glb}, no primitive
declares \code{extras.uos\_fdi} and none declares the \code{\_REGION\_ID} attribute. This
verifies where layer 3 \emph{is}, which is the other half --- and without it check 17
passes on a container whose layer-1 scene is full of model output.\\
