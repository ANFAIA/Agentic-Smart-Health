...
(\S\ref{sec:container}).\\
\code{serie-completa.uos} & valid & A container that \emph{carries} the DICOM series, slice
...
\code{serie-corte-faltante.uos} & error & A slice is missing, and the validator must name
\emph{which} (\S\ref{sec:volume}).\\
\code{serie-corte-alterado.uos} & error & A slice with the same name and size and one byte
changed.\\
\code{serie-corte-sobrante.uos} & error & A slice the manifest does not declare in
\code{parts[]}. Extra is as serious as missing: nobody knows where it came from.\\
\code{serie-corte-deidentificado.uos} & valid, warns & A slice whose tags were cleaned: it
