...
adopted, and to a result that constrains how it should be used: \textbf{re-fitting a single
acquisition recovers no detail below its sampling pitch}. What it adds is the ability to allocate representational capacity
...
\begin{finding}{What we measure today: re-fitting one acquisition reallocates capacity, it does not create resolution}
...
the mechanism of multi-view super-resolution and the reason a photometric field fitted to
many photographs resolves structure no one photograph resolves. That regime is not measured
here, and nothing below should be read as a claim about it.
\end{finding}
...
\S\ref{sec:results} --- one measured median per crown per third, plus gingiva --- rather
...
compute (\S\ref{sec:results}). Against a piecewise-constant target the image loss cannot
...
and supervise on the photographs' own pixels, which is a prerequisite (\S\ref{sec:results})
...
different resolution, and not for enhancement of a single acquisition, which our
measurements show it does not deliver.
