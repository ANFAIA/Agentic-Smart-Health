...
its \code{\$id},
\url{https://raw.githubusercontent.com/ANFAIA/Agentic-Smart-Health/uos-spec-v0.3/schemas/uos-manifest-0.3.schema.json}
...
\begin{itemize}
  ...
  (\S\ref{sec:artifacts}). A \textbf{released} tag \MUSTNOT{} be moved; a correction is a new
  one. \textbf{What counts as released is the \code{Status} on the title page}, and that
  sentence was missing until a tag had to be moved to find out. While a version declares
  itself \code{Draft}, its tag is a \emph{pre-release pointer}: it exists so the \code{\$id}
  resolves during development, and it \MAY{} be moved. From the moment the title page stops
  saying \code{Draft}, the tag freezes and a correction \MUSTNOT{} reuse the identifier.

  ⚠️ \textbf{This has been exercised twice, and both are recorded rather than left implicit.}
  \code{uos-spec-v0.3} was moved on 2026-09-24, one day after it was first pushed, because
  what it published was a validation-report schema requiring \code{"0.2"} while the reference
  ...
  not resolve, because a 404 is visible. It was moved a second time the same day, this time
  without changing a byte of what it publishes: the first push had put it on a working
  branch rather than on the merge commit in \code{main}, which is where the repository's
  release rules say a version tag belongs. A validator check (\S\ref{sec:conformance}) now
  compares what a tag publishes against the file in the tree, so the next occurrence is found
