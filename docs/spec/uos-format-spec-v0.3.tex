...
% =============================================================================
%  UOS — Unified Oral Scene. Format Specification v0.3
...
% La version del formato, escrita UNA vez. La portada, el encabezado de pagina y el nombre
% de los esquemas la repetian a mano, y al subir 0.2 -> 0.3 el bump arreglo la portada y se
% dejo el `\fancyhead`: todas las paginas del PDF anunciaban una version que el documento ya
% no era. `scripts/docs_sync.py` ata esto a `UOS_VERSION` del contrato.
\newcommand{\uosver}{0.3}

\pagestyle{fancy}\fancyhf{}
\fancyhead[L]{\footnotesize\color{uosgrey}UOS: Unified Oral Scene v\uosver}
...
{\color{uosblack}\LARGE Version \uosver}\\[1.4cm]
...
\textbf{Manifest schema} & \code{schemas/uos-manifest-0.3.schema.json}\\
...
\textbf{Licence} & Apache License 2.0 (\code{LICENSE}, repository root), covering\\
 & \textbf{this document and the reference implementation}\\
 & \textbf{alike}; neither is more reusable.\\
\textbf{Extension prefix} & \code{histora\_}, the only prefix this document reserves (\S\ref{sec:extensions})\\
\textbf{Produced under} & Versions 0.2 and 0.3 were written during the Becas de Verano ANFAIA\\
 & 2026, on a clinical case contributed by Histora.\\
\textbf{Version} & \uosver{} --- last revised \today\\
\end{tabular}

\vspace{0.8cm}
{\color{uosgrey}\small Every figure was measured on a real clinical case
...
\S\ref{sec:limits}; \S\ref{sec:artifacts} where the schema for each
version lives. This document is the contract. An implementation that follows it
...
\code{schemas/uos-manifest-0.3.schema.json} in the reference repository and retrievable at
its \code{\$id},
\url{https://raw.githubusercontent.com/ANFAIA/Agentic-Smart-Health/uos-spec-v0.3-draft/schemas/uos-manifest-0.3.schema.json}
--- pinned to a tag, so the identifier always names the same document. \textbf{While this
document says \code{Status: Draft} that tag carries the \code{-draft} suffix and may move}
(\S\ref{sec:versioning}); on publication the suffix goes and the tag is created once.  It is
...
A reader should not go looking for a file that does not accompany the document. What
\emph{is} distributable is the synthetic conformance fixture of \S\ref{sec:fixture}.\\
\bottomrule
\end{longtable}
...
\code{histora\_clinical} (\S\ref{sec:clinical})\\
...
derived/seg_gaussians.field.bin       <- one code per Gaussian  (layer 3)
derived/seg_gaussians.field.meta.json
derived/seg_gaussians.composite.bin   <- one file per layer: each has
derived/seg_gaussians.composite.meta.json   its own count and order
...
directory, not re-exporting anything. What the separation buys is the ability to say that
layers 1 and 2 are \textbf{not Software as a Medical Device} (\textbf{SaMD}) --- software
whose intended purpose is medical, and which therefore falls under the EU Medical Device
Regulation or the FDA's --- and that only the layer-3 modules are. A claim like that has to
be enforceable or it is not worth making. That property only holds if it is enforced --- which
...
\paragraph{Normative: what traversing the graph means.} A registration
\code{A}~$\to$~\code{B} states \code{p\_B = T . p\_A} with points as column vectors. The
graph is walked \emph{undirected}, so three rules are needed and this specification fixes
them.

\begin{enumerate}
  \item \textbf{Crossing an edge backwards inverts its matrix.} Using
  \code{A}~$\to$~\code{B} to place something expressed in \code{B} means applying
  $T^{-1}$. The transform is rigid, so the inverse always exists.
  \item \textbf{A path composes right to left.} For \code{F}~$\to$~\code{A}~$\to$~\code{C}
  the matrix is $T_{AC} \cdot T_{FA}$: the first hop is applied first, so it sits on the
  right.
  \item \textbf{When several paths exist, a reader \MUST{} take the one with the fewest
  edges}, breaking a tie by the ascending sequence of registration ids. Error propagates
  along the path, so two hops are not worth one, and the choice cannot be left to
  iteration order --- two readers would place the same asset differently.
\end{enumerate}

Paths that disagree are \textbf{not} averaged. Averaging would produce a pose that no
registration claims and hide the disagreement inside it, which is the one thing this format
exists to prevent. The reader takes the shortest path and check 15b reports the spread,
which is a measured bound on what a composition across those frames is worth.

The reference implementation resolves this in \code{uos.marcos}:
\code{resuelve\_al\_canonico} returns the composed transform and
\code{discrepancia\_maxima} the spread.
...
\vspace{6px}
The asset's own \code{sha256} is a \textbf{digest over the parts}, defined here because
the format needs writer and validator to compute it identically. It runs over
\emph{clinical identity} when every part declares it, and falls back to names and file
hashes when they do not, so that a container written before identity was carried still
verifies. \S\ref{sec:volume} says why identity is the better key:

\begin{lstlisting}[style=py]
def digest_of_parts(parts):
    h = hashlib.sha256()
    if all(p.sop_instance_uid and p.pixel_data_sha256 for p in parts):
        for p in sorted(parts, key=lambda x: x.sop_instance_uid):
            h.update(f"{p.sop_instance_uid}\0{p.pixel_data_sha256}\n".encode())
        return h.hexdigest()                          # identity: the clinical series
    for p in sorted(parts, key=lambda x: x.name):     # fallback: one serialisation
...
\paragraph{Locators: where the original lives, as a hint.} A content address says
\emph{which} file an asset is and proves it when someone holds it. It does not say
\emph{who} holds it, so \code{sha256:<hex>} names a file nobody can go and fetch. An asset
\MAY{} therefore carry \code{locators}, each with a \code{kind} --- \code{dicomweb} for a
QIDO-RS or WADO-RS query by Series Instance UID, \code{ae\_title} for a PACS reachable over
classic DIMSE, \code{url}, or \code{opaque} for a reference that only means something
inside one institution.

They are \textbf{hints and never a contract}. A reader \MUST{} open and validate a
container without resolving any of them, identity is decided by the hash and never by the
route, and an empty list is \textsc{not declared} rather than ``there is nowhere''. A
locator on an asset that is not \code{external} is an error: it would describe where to
find something the container already carries.

\begin{finding}{A locator can be the leak, so it declares whether it is one}
The route to the system that holds the original usually identifies the patient, the site,
or both --- a patient directory in a URL, an AE title naming the clinic, a hostname that is
the practice. A container can be impeccably de-identified and still carry the way back
written inside it.

So \code{identifying} has \textbf{no default}. A writer states whether the locator itself
carries identity, because leaving it silent would make ``it does not identify'' and ``nobody
looked'' indistinguishable, which this format does not permit anywhere else. Check 17o
makes an \code{identifying} locator an error in a container declaring \code{pseudonymized}
or \code{anonymized}, and it costs the container \code{UOS-Distributable} in every case.
\end{finding}
...
\textbf{reconstruction}: the \code{histora\_reversible} extension regenerates an arch mesh from
...
% generado: tabla-Manifest — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{uos\_version} & string & default \code{0.3} & Format version this container declares. A reader \MUST{} check it before anything else.\\
\code{case\_id} & string & \MUST{} & Stable case identifier, conventionally \code{urn:uuid:<v5>}. It \MUSTNOT{} encode patient identity.\\
\code{created} & date-time & --- & When this manifest was written, RFC 3339 in UTC.\\
\code{generator} & map str$\to$string & \MUST{} & Who wrote this container: name and version of the writer.\\
\code{phi\_state} & enum & \MUST{} & What identifiable content the container carries. It is a statement about the whole container, and a reader decides what it may do with it from here.\\
\code{deidentification} & \code{Deidentification} & --- & What de-identification was applied, to which assets, and with what tool. \code{null} means none was declared, which is not the same as none being needed.\\
\code{purpose\_of\_use} & enum & --- & What this container was assembled for. It bounds what a recipient may do with it, and is checked against the subject's consent.\\
\code{subject} & \code{Subject} & \MUST{} & The patient, always by pseudonym.\\
\code{canonical\_frame} & \code{Frame} & \MUST{} & The frame every other frame must be reachable from. It is the geometric hub of the case.\\
\code{frames} & \code{Frame}[] & --- & Every coordinate frame named in the case, besides the canonical one.\\
\code{visits} & \code{Visit}[] & --- & The temporal axis: one entry per clinical encounter.\\
\code{assets} & \code{Asset}[] & --- & The data plane: every file the container carries or references. The manifest is exhaustive, so a reader never walks the ZIP to discover content.\\
\code{registrations} & \code{Registration}[] & --- & The relations: the transforms that place each frame with respect to another, each with its residual.\\
\code{occlusion} & enum & --- & How the two arches relate, for a case that has both. Silence is never 'there is none': a two-arch case \MUST{} declare a registration or say why it cannot.\\
\code{fhir\_map} & map str$\to$\code{FHIRResource} & --- & Which FHIR resource each asset maps to, so a clinical system can ingest the case without knowing this format.\\
\code{extensions} & map str$\to$\code{Extension} & --- & Extensions this container declares, by name.\\
\code{extensions\_used} & string[] & --- & Names of extensions present. A reader that does not implement one still opens the case and can say what it left unread.\\
\code{extensions\_required} & string[] & --- & Names of extensions a reader \MUST{} implement to open the case correctly. Listing one here refuses readers, so it is for cases that would be misread otherwise.\\
\code{provenance} & \code{Provenance} & --- & The link to the previous version of this case, and to the chain that records every version.\\
\bottomrule
\end{longtable}
% /generado: tabla-Manifest

\subsection{Asset}

% generado: tabla-Asset — no editar a mano
\begin{longtable}{L{3.2cm} L{2.9cm} C{1.4cm} L{6.8cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{id} & string & \MUST{} & Stable identifier of the asset within this container, unique across \code{assets[]}.\\
\code{kind} & enum & \MUST{} & What the asset is, which fixes how a reader should treat it.\\
\code{visit} & string & \MUST{} & Id of the visit this asset belongs to.\\
\code{uri} & string & \MUST{} & Path inside the ZIP for a carried asset, or \code{sha256:<hex>} for a referenced one. A referenced original occupies no path at all.\\
\code{media\_type} & string & \MUST{} & IANA media type of the payload.\\
\code{sha256} & string & \MUST{} & SHA-256 of the payload, lowercase hex. For a directory asset it is the digest over \code{parts[]}, not over concatenated bytes.\\
\code{bytes} & integer & \MUST{} & Size of the payload in bytes; the sum over \code{parts[]} for a directory asset.\\
\code{frame} & string & \MUST{} & Id of the coordinate frame this asset's coordinates are expressed in. It \MUST{} be reachable from the canonical frame.\\
\code{acquisition} & \code{Acquisition} & --- & When and on what equipment it was captured.\\
\code{load\_priority} & integer & default \code{30} & Hint for a reader fetching by HTTP range: lower loads first. A hint, never a requirement.\\
\code{regulatory} & \code{Regulatory} & --- & Regulatory layer and clearances of this asset's content.\\
\code{projection} & \code{Projection} & --- & Projection geometry, for 2D radiographs only. \code{null} for everything else.\\
\code{parts} & \code{Part}[] & --- & One entry per file when the asset is a directory, such as a DICOM series. This is what lets a validator say which slice is wrong instead of 'the series does not match'.\\
\code{sidecar\_uri} & string & --- & Path to a JSON sidecar describing the payload, so a reader can inspect it without a parser for its format.\\
\code{derived\_from} & string[] & --- & Ids of the assets this one was derived from. Required for anything under \code{derived/}: an inference with no stated input cannot be audited.\\
\code{series\_instance\_uid} & string & --- & DICOM Series Instance UID \code{(0020,000E)}, so a recipient holding the series by another channel can tell it is this one.\\
\code{study\_instance\_uid} & string & --- & DICOM Study Instance UID \code{(0020,000D)}.\\
\code{external} & bool & default \code{False} & True when the asset is referenced and not carried. Acquired originals \MUST{} be external; their \code{uri} is then a content address and never a path.\\
\code{locators} & string & --- & Where to find this asset when it is referenced and not carried. Hints, never a contract: a reader verifies by hash and \MUST{} open the container without resolving any of them. An empty list is NOT DECLARED.\\
\bottomrule
\end{longtable}
% /generado: tabla-Asset
...
% generado: tabla-Registration — no editar a mano
\begin{longtable}{L{4.2cm} L{2.6cm} C{1.2cm} L{6.3cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{id} & string & \MUST{} & Stable identifier of this registration edge.\\
\code{source\_frame} & string & \MUST{} & Id of the frame points are taken from.\\
\code{target\_frame} & string & \MUST{} & Id of the frame points are taken to.\\
\code{transform\_4x4\_row\_major} & number[] & \MUST{} & The rigid transform as sixteen numbers, row-major, millimetres. glTF expects column-major, so a reader targeting glTF \MUST{} transpose.\\
\code{method} & string & \MUST{} & How the transform was obtained, e.g. \code{icp\_surface} or \code{manual}.\\
\code{rms\_error\_mm} & number & --- & Root-mean-square residual of the fit, in millimetres. It is measured on the points that were fitted, so it is systematically optimistic.\\
\code{max\_error\_mm} & number & --- & Largest residual over the fitted points, in millimetres. An RMS hides a local failure that this exposes.\\
\code{target\_registration\_error\_mm} & number & --- & Registration error measured at points that did NOT take part in the fit, in millimetres. This is the error a clinical measurement across the seam actually inherits.\\
\code{tre\_region} & string & --- & Where the target registration error was measured. A TRE without its region is not comparable with another.\\
\code{fit\_for} & enum[] & --- & What this registration is accurate enough for, declared rather than inferred from the residual. An empty list is 'not assessed'.\\
\code{computed} & date-time & --- & When the transform was computed, RFC 3339.\\
\code{operator} & string & --- & Who or what produced the estimate. A machine here with an empty \code{verified\_by} means provisional.\\
\code{verified\_by} & string & --- & Who checked it. \code{null} means nobody did, and a viewer \MUST{} present the edge as provisional.\\
\code{regulatory} & \code{Regulatory} & --- & Regulatory layer of the transform itself, when it was produced by a model rather than fitted.\\
\bottomrule
\end{longtable}
% /generado: tabla-Registration
...
A frame is one coordinate system, named. It is \textbf{per acquisition and not per device}: two scans of the same patient on different visits are different frames even off the same scanner, because sharing a frame asserts the two clouds are already in one space, and between visits that is false.

% generado: tabla-Frame — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{id} & string & \MUST{} & Stable identifier of the frame within this container.\\
\code{description} & string & --- & Human-readable name of the frame. Free text, and never load-bearing.\\
\code{units} & string & default \code{mm} & Length unit of coordinates in this frame. Millimetres everywhere in this version.\\
\code{handedness} & string & default \code{right} & Handedness of the axes, \code{right} or \code{left}. A reader that ignores it renders a mirrored case that looks correct.\\
\code{anatomical} & enum & --- & Anatomical axis convention, such as \code{LPS} or \code{RAS}. \code{null} is NOT DECLARED, and is not the same as a device convention.\\
\code{dicom\_frame\_of\_reference\_uid} & string & --- & DICOM Frame of Reference UID \code{(0020,0052)}. Read from the series, never invented: it is the only frame identifier that means the same thing outside this container.\\
\bottomrule
\end{longtable}
% /generado: tabla-Frame

\subsection{Part}

One file inside a directory asset. \code{parts[]} is what lets a validator say \emph{which} slice is wrong instead of reporting that the series does not match, and it travels even when the series itself does not.

% generado: tabla-Part — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{name} & string & \MUST{} & Path of this file inside the directory asset. Names are ordering data for a DICOM series, so they are carried even though identity does not depend on them.\\
\code{sha256} & string & \MUST{} & SHA-256 over the whole file, lowercase hex. Proves these exact bytes.\\
\code{bytes} & integer & \MUST{} & Size of the file in bytes.\\
\code{sop\_instance\_uid} & string & --- & DICOM SOP Instance UID \code{(0008,0018)}. The identity DICOM itself defines, and what survives de-identification when UIDs are retained.\\
\code{pixel\_data\_sha256} & string & --- & SHA-256 over the value of the PixelData element \code{(7FE0,0010)} alone, without the header. De-identification does not touch it, so it separates 'a different slice' from 'the same slice, cleaned'.\\
\bottomrule
\end{longtable}
% /generado: tabla-Part
...
One clinical encounter. Assets name the visit they belong to, which is what makes a longitudinal question arithmetic rather than a research project.

% generado: tabla-Visit — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{id} & string & \MUST{} & Stable identifier of the visit within this container.\\
\code{date} & string & \MUST{} & Date of the encounter. Shifted when \code{deidentification.date\_shift\_days} is set.\\
\code{label} & string & --- & Human-readable name for the encounter. Free text.\\
\bottomrule
\end{longtable}
% /generado: tabla-Visit

\subsection{Subject}

The patient, and only ever by pseudonym.

% generado: tabla-Subject — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{pseudonym} & string & \MUST{} & The subject's pseudonym. Always a pseudonym: a direct identifier \MUSTNOT{} appear here.\\
\code{fhir\_patient} & string & --- & Reference to a FHIR \code{Patient} resource held elsewhere. A pointer, never the record.\\
\code{consent} & \code{Consent} & --- & What the subject consented to, and for which purposes.\\
\bottomrule
\end{longtable}
% /generado: tabla-Subject

\subsection{Consent}

What the subject agreed to, and for which purposes. The container carries the pointer to the FHIR resource, never the document itself.

% generado: tabla-Consent — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{fhir\_consent} & string & --- & Reference to the FHIR \code{Consent} resource that records it. The container carries the pointer, never the document.\\
\code{scope} & enum[] & --- & Purposes the subject consented to. An empty list is 'not declared'.\\
\code{obtained} & date-time & --- & When consent was obtained, RFC 3339.\\
\bottomrule
\end{longtable}
% /generado: tabla-Consent

\subsection{Regulatory}

The layer of every asset's content, and what each regulator says about it. This is the field the whole inference plane rests on.

% generado: tabla-Regulatory — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{layer} & integer & default \code{1} & Regulatory layer of the content: 1 acquired or transcribed, 2 computed deterministically from layer 1 with no trained model, 3 model output. Layer 3 \MUST{} live under \code{derived/}.\\
\code{clearances} & \code{Clearance}[] & --- & What each regulator says about this asset. An empty list is 'nothing declared', not 'cleared everywhere'.\\
\bottomrule
\end{longtable}
% /generado: tabla-Regulatory

\subsection{Clearance}

What \emph{one} jurisdiction says about one asset under one regime. A status with no jurisdiction means nothing: \code{investigational} is a claim before a named regulator, not a property of a file.

% generado: tabla-Clearance — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{jurisdiction} & string & \MUST{} & Regulator the statement is made before, e.g. \code{EU} or \code{US}. A status without a jurisdiction means nothing.\\
\code{regime} & string & \MUST{} & Regulatory framework under that jurisdiction, e.g. \code{MDR} or \code{FDA-510k}.\\
\code{status} & string & \MUST{} & What this jurisdiction says about the asset under that regime.\\
\code{reference} & string & --- & Dossier or clearance number when one exists. \code{null} is 'not recorded here', never 'there is none'.\\
\bottomrule
\end{longtable}
% /generado: tabla-Clearance

\subsection{Deidentification}

What was done to remove identity, to which assets, and with which tool. Without it \code{phi\_state} is an assertion a recipient has to take on trust.

% generado: tabla-Deidentification — no editar a mano
\begin{longtable}{L{3.8cm} L{2.7cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{profile} & string & \MUST{} & DICOM PS3.15 Annex E confidentiality profile applied, by its standard name.\\
\code{options} & string[] & --- & Named options applied on top of the profile, e.g. \code{CleanRecognizableVisualFeatures} for defacing. This is where a reader learns whether the face was removed.\\
\code{applied\_to} & string[] & --- & Asset ids the de-identification was applied to. Assets not listed were not processed.\\
\code{tool} & \code{Tool} & --- & Software that performed it, so a recipient can tell a documented pipeline from a manual pass.\\
\code{date\_shift\_days} & integer & --- & Constant offset applied to every date, in days. Needed to compare two visits after shifting; \code{null} means dates were not shifted.\\
\code{note} & string & --- & Free text for anything the fields above cannot express. Not machine-readable by design.\\
\bottomrule
\end{longtable}
% /generado: tabla-Deidentification

\subsection{Tool}

The software that performed an operation, so a recipient can tell a documented pipeline from a manual pass.

% generado: tabla-Tool — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{name} & string & \MUST{} & Name of the software that performed the operation.\\
\code{version} & string & --- & Its version string, as the tool reports it.\\
\code{sha256} & string & --- & Digest of the tool binary or image, when the writer can compute it.\\
\bottomrule
\end{longtable}
% /generado: tabla-Tool
...
The equipment, with fixed keys. \textbf{No serial number}: it identifies a machine, and through the machine a site.

% generado: tabla-Device — no editar a mano
\begin{longtable}{L{3.8cm} L{2.7cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{manufacturer} & string & --- & Equipment manufacturer. Fixed key rather than free text so two containers can be compared.\\
\code{model} & string & --- & Equipment model name.\\
\code{software\_version} & string & --- & Version of the software running on the equipment. No serial number is carried: it identifies a machine, and through it a site.\\
\bottomrule
\end{longtable}
% /generado: tabla-Device

\subsection{Acquisition}

When an asset was captured and on what. \code{null} time means the original did not record it, not that it is unknown to us.

% generado: tabla-Acquisition — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{time} & date-time & --- & When the asset was captured, RFC 3339. \code{null} when the original did not record it.\\
\code{device} & \code{Device} & --- & Equipment that captured it.\\
\bottomrule
\end{longtable}
% /generado: tabla-Acquisition

\subsection{Projection}

Projection geometry, for 2D radiographs only. A panoramic and a bitewing are different measurements and must not look alike.

% generado: tabla-Projection — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{type} & string & \MUST{} & Projection geometry of a 2D image, e.g. \code{panoramic} or \code{bitewing}.\\
\code{fdi\_targets} & string[] & --- & Teeth the projection is aimed at, in FDI/ISO-3950. An empty list is 'not declared', never 'no teeth'.\\
\bottomrule
\end{longtable}
% /generado: tabla-Projection

\subsection{Provenance}

The link to the previous version of this case. A new version is written rather than the old one edited, and the chain is what makes the history verifiable by whoever receives it without trusting the sender.

% generado: tabla-Provenance — no editar a mano
\begin{longtable}{L{4.5cm} L{2.5cm} C{1.4cm} L{5.9cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{prev\_manifest\_sha256} & string & --- & SHA-256 of the previous version's manifest. This is what makes the history a chain rather than a claim.\\
\code{chain} & string & --- & Path to the append-only chain file. Present without \code{prev\_manifest\_sha256} means a history exists that this manifest does not point into.\\
\bottomrule
\end{longtable}
% /generado: tabla-Provenance
...
selection inside an imaging study --- instances, frames, regions. This version declares R4, and
...
\paragraph{Sites that are not teeth.} An FDI code labels teeth only, so there was no
...
\code{target\_registration\_error\_mm} --- the \textbf{Target Registration Error}
(\textbf{TRE}), measured at points \emph{not} used to compute
...
identifies a coordinate system globally and uniquely --- the
\textbf{Frame of Reference UID}, tag \code{(0020,0052)}, which every CBCT series carries --- and a reader that
...
% generado: tabla-FHIRResource — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{resource\_type} & string & \MUST{} & FHIR resource type this asset maps to, e.g. \code{Observation} or \code{ImagingSelection}.\\
\code{resource} & string & --- & Reference to the resource instance, when one exists outside the container.\\
\code{note} & string & --- & Why this mapping was chosen. Free text, for the reader who disagrees with it.\\
\bottomrule
\end{longtable}
% /generado: tabla-FHIRResource
...
why \code{histora\_gs\_measured} exists at all: it is the part of this format the base draft
...
Candidate} at the time of writing, written against the glTF 2.0 spec. UOS emits it and
...
before storage. \textbf{UOS does not emit \code{COLOR\_0}}, which means a glTF viewer
...
same convention the \code{histora\_reversible} extension uses when sampling colour per vertex,
and it is cited rather than redefined.

\paragraph{Underscore attributes.} \code{\_AO} uses the underscore prefix glTF reserves
for what its specification does not define. A conformant viewer ignores it and draws
correctly; the UOS viewer uses it for shading. Per-Gaussian tooth identity is \emph{not}
carried here: it is layer 3 and lives in \code{derived/} (\S\ref{sec:picking}). That they are non-standard is precisely why the \code{histora\_gs\_measured}
...
Declared by extension \code{histora\_gs\_measured}. One descriptor per Gaussian layer,
column by column.
...
\code{profile} & string & e.g. \code{histora-twin/1.0}, \code{histora-gs-appearance/1.0}.\\
...
(\code{linear}\,|\,\code{log}), \code{measured}, \code{derived\_from}, \code{meaning},
\code{vocabulary}.\\
...
{ "role": "twin density field", "measured": true, "profile": "histora-twin/1.0",
  ...
  "subsampling": {"step_voxels": [3,3,1], "from": 12072785, "to": 1341990,
                  "method": "stride",
                  "occupancy": {"threshold": ..., "unit": "grey_value",
                                "note": "one primitive per voxel above this threshold.
                                         Raising it erases dentine before enamel, so it
                                         changes WHICH TISSUE appears. Not HU unless the
                                         volume sidecar declares `calibrated_hu: true`"}},
  "columns": [ {"name":"x","unit":"mm","scale":"linear","measured":true,  ...},
               {"name":"scale_0","unit":"mm","scale":"linear","measured":true, ...},
               {"name":"density","unit":"normalised_sigma","scale":"linear",
                "measured":true, ...} ] }

// scene/appearance.gs.json -- the trained appearance embedded in scene.glb
{ "role": "real appearance trained with gsplat", "measured": false,
  "profile": "histora-gs-appearance/1.0", "frame": "frame.ios_master", "n_primitives": 113827,
...
\code{value\_range} & number[2]\,|\,null & Rescaled $[\min,\max]$. Taken from
\code{Smallest/LargestImagePixelValue} when the series declares them, and otherwise
\MUST{} be measured during the \code{parts[]} hash, which reads every byte anyway.
\code{null} only when the writer never held the series, with a warning: a fabricated
CBCT range would feed straight into a viewer's windowing.\\
...
\vspace{6px}
Camera \code{pose} is explicitly optional in this version and the reference implementation does not
...
\code{clinical/observations.json}, declared by extension \code{histora\_clinical}.

\paragraph{Why it exists.} The format's core gives per-tooth clinical attributes no place: the FHIR
...
\paragraph{Signatures.} \code{provenance/signatures/} is reserved. \textbf{This version does not
...
knows whether it can read it fully, partly, or not at all. An earlier draft did not
...
below, a reader that does not implement \code{histora\_clinical} still opens the case, still
...
% generado: tabla-Extension — no editar a mano
\begin{longtable}{L{3.5cm} L{3.0cm} C{1.4cm} L{6.4cm}}
\toprule
\textbf{Field} & \textbf{Type} & \textbf{Req.} & \textbf{Description}\\
\midrule
\endhead
\code{name} & string & \MUST{} & Prefixed extension name. This specification reserves \code{histora\_} and defines no other.\\
\code{version} & string & \MUST{} & Version of the extension, independent of the format version.\\
\code{uri} & string & --- & Path to the extension's payload inside the container, declared as an asset like any other.\\
\code{schema\_id} & string & --- & Identifier of the schema its payload validates against.\\
\code{description} & string & --- & One line on what the extension adds, so a reader that skips it can say what it skipped.\\
\bottomrule
\end{longtable}
% /generado: tabla-Extension
...
\midrule
\endhead
\code{histora\_clinical} & 1.0 & Per-tooth clinical attributes (pH, roots, canals, findings,
measured colour) with per-value extraction provenance. \S\ref{sec:clinical}. Carried at
\code{clinical/observations.json}.\\
\code{histora\_gs\_measured} & 1.0 & The \code{.gs.json} descriptor per Gaussian layer:
...
\code{histora\_reversible} & 1.0 & Declares that an arch mesh with per-vertex colour, FDI code
and a \code{measured} column can be \emph{regenerated} from \code{asset.scene}: geometry
and FDI from \code{derived/seg\_teeth.bin}, colour from the
...
\begin{enumerate}
  \item Pick a prefixed name (\code{<vendor>\_<thing>}); do not squat on \code{histora\_} or
  \code{KHR\_}.
...
\subsection{The validation report}\label{sec:report}

A validator that only prints prose forces anything downstream to run regular expressions
over sentences that change between versions. So the report has a shape, published as
\code{schemas/uos-validation-report-0.3.schema.json} and validated against it by the
reference implementation on every case of the conformance fixture.

\begin{lstlisting}[style=js]
{ "uos_validation_report": "0.3",
  "valid": false,
  "findings": [
    { "code": "UOS-E-005", "severity": "error", "check": "5",
      "path": "assets[asset.scene]",
      "message": "asset asset.scene: declared sha256 is not the file's" } ],
  "levels": ["UOS-Core"], "distributable": false,
  "not_distributable_because": ["phi_state is identified"],
  "version": 2, "views": 3, "external_count": 1 }
\end{lstlisting}

\code{path} locates the finding --- a manifest path such as \code{assets[<id>]}, or an
entry inside the ZIP --- and is \code{null} when the finding is about the container as a
whole. \code{valid} is \code{true} exactly when no finding has severity \code{error}:
warnings never invalidate.

\paragraph{Every finding carries a stable code.} A validator \MUST{} report each
finding with the identifier of the check that produced it, formed as
\code{UOS-E-<check>} for an error and \code{UOS-W-<check>} for a warning, with the numeric
part padded to three digits: check 5 reports \code{UOS-E-005}, check 17b reports
\code{UOS-E-017b}. The message is written for whoever is debugging and \MAY{} change; the
code is the contract, and is what a program should branch on.
...
15b & W & More than one path of registrations reaches the canonical frame
from a frame an asset uses, and they disagree by more than $10^{-3}$. The reader still gets
a defined answer; the spread is what a measurement across the seam inherits.\\
...
that inconsistency does not parse at all and check 4 reports it. Listed here because the
...
row-major; confusing them does not fail, it places the cloud rotated and mirrored.\\18 & E & Extensions: \code{used} $\subseteq$ \code{extensions}; \code{required} $\subseteq$
\code{used}; every non-null extension \code{uri} is a declared asset.\\
17n & W & \textbf{Declared, not assumed}: no \code{occlusion} on a case that could
have one; a layer-3 asset with no \code{clearance}; a registration with an empty
\code{fit\_for}; a canonical frame with no anatomical convention. None of these invalidate
the container --- each says that something a reader must not guess was left unsaid.\\
17o & E/W & Locators (\S\ref{sec:external}): one on a non-external asset is an
error; an \code{identifying} one in a container declaring \code{pseudonymized} or
\code{anonymized} is an error; and any locator warns, because it points outside the
container and nobody has checked that it answers.\\
19 & E/W & Extensions declared but unused; and one warning per required extension, since a
...
nuevo = asset_de(                       # hashes and measures the real file
    Path("/data/photo_09.jpg"), uri="",  # discarded: external assets are
    id_="asset.img_009", kind=Clase.IMAGE2D, visit="v1",   # named by content
    frame="frame.ios_master", media_type="image/jpeg",     # address, not path
    external=True,
)
...
A clinical photograph is an \textbf{acquired original}, so it does not travel
(\S\ref{sec:external}): \code{external=True} makes the writer discard the path
you passed and name the asset \code{sha256:<hex>} instead. Writing it to a path
inside the ZIP --- \code{images/img\_009.jpg} or any other --- is the writer defect
\S\ref{sec:external} describes, and there is no reserved directory for one.
...
This is what \code{histora\_reversible} declares, and the practical answer to ``why keep the
...
\begin{enumerate}
  ...
  \emph{mesh} primitive. The per-vertex FDI code is \emph{not} in the scene: read it from
  \code{derived/seg\_teeth.bin} (\S\ref{sec:derived}), and if that directory was stripped,
  the mesh regenerates without it.
...
\midrule
\endhead
\textbf{Minor}, from 1.0 on & A reader written for the earlier version \MUST{} still be
...
\textbf{Minor}, before 1.0 & \textbf{No compatibility is promised.} The format is a draft
with no second implementation to strand, and pretending otherwise would be the more
dangerous claim: a reader trusting minor compatibility would open a 0.3 container with 0.2
assumptions and get a plausible wrong answer. A pre-1.0 minor \MAY{} rename a value, change
what a field means or remove one --- and when it does it \MUST{} say which, case by case,
in the changelog. Version 0.3 did all three.\\
...
\begin{itemize}
  ...
  is pinned to a tag so the identifier always names the same document
  (\S\ref{sec:artifacts}). A published tag \MUSTNOT{} be moved; a correction is a new one.

  \textbf{Two tags, because the identifier has two jobs and they pull opposite ways.} One is
  to resolve the \code{\$id} \emph{while the version is being written}, which requires
  following the content; the other is to mark the publication, which requires never moving.
  A single tag cannot do both.

  \begin{longtable}{L{4.6cm} L{10.6cm}}
  \toprule
  \textbf{Tag} & \textbf{What it is}\\
  \midrule
  \endhead
  \code{uos-spec-v<version>-draft} & The \textbf{pre-release pointer}. It is what the
  \code{\$id} names while the title page says \code{Status: Draft}, so an implementer can
  resolve the schema during development. It \MAY{} be moved, and its name says so --- which
  is the whole point: a moving identifier that announces it moves misleads nobody.\\
  \code{uos-spec-v<version>} & The \textbf{release tag}. Created \emph{once}, when the
  version is published, and never moved. Until then it \MUSTNOT{} exist, because a name
  that promises immutability should not be pointing at a draft.\\
  \bottomrule
  \end{longtable}

  ⚠️ \textbf{This separation is the outcome of getting it wrong, and the record stays.}
  \code{uos-spec-v0.3} was created from a working branch, before the pull request carrying it
  was merged, and then moved twice on 2026-09-24. The first move was because what it
  published was a validation-report schema requiring \code{"0.2"} while the reference
  implementation emitted \code{"0.3"} --- so resolving the \code{\$id} handed an outside
  implementer the artifact that rejects our own reports, which is worse than a tag that does
  not resolve, because a 404 is visible. The second put it on the merge commit in
  \code{main}, where the repository's release rules say a version tag belongs.

  Both moves were symptoms rather than the disease: one name was carrying both jobs, so every
  correction to a draft had to violate the rule above in order to keep the \code{\$id}
  resolving. With the two separated, the rule needs no exception --- which is why the
  exception a previous revision of this section carried has been withdrawn. A validator check
  (\S\ref{sec:conformance}) compares what a tag publishes against the file in the tree, so a
  tag that drifts from its schema is found before anybody downloads it.
  \item Extensions, independently. \code{histora\_clinical} at 1.0 does not become 1.1 because
...
It ships two \textbf{valid} containers, one that is valid but must warn, and ten broken ones, each carrying \emph{a single}
defect, with the class of result each should produce written beside it in
\code{expected.json}. An implementer runs their validator over the directory and compares.
Accepting one of the broken ones, or rejecting the valid one, tells them precisely what
they are missing.

\begin{longtable}{L{6.3cm} L{2.2cm} L{6.5cm}}
\toprule
\textbf{Case} & \textbf{Expects} & \textbf{What it exercises}\\
\midrule
\endhead
\code{valid.uos} & \code{valid} & The reference container.\\
\code{undeclared-entry.uos} & \code{error} & A file the manifest does not name: no hash
accrediting it, no regulatory layer (\S\ref{sec:container}).\\
\code{hash-mismatch.uos} & \code{error} & A carried asset whose declared \code{sha256} is not
its bytes'.\\
\code{manifest-not-first.uos} & \code{error} & \code{manifest.json} is not the first
physical entry (\S\ref{sec:container}).\\
\code{layer-3-outside-derived.uos} & \code{error} & Layer 3 outside \code{derived/}
(\S\ref{sec:derived}).\\
\code{provisional-registration.uos} & \code{warning} & An automatic registration with no
\code{verified\_by}: the container is valid and a viewer must present it as provisional
(\S\ref{sec:manifest}).\\
\code{higher-minor-new-field.uos} & \code{valid-with-warning} & A higher minor with an unknown
field: it is ignored and \emph{named}, not rejected (\S\ref{sec:versioning}).\\
\code{compressed.uos} & \code{error} & DEFLATE instead of STORE, so range access does not work
(\S\ref{sec:container}).\\
\code{series-complete.uos} & \code{valid} & A container that \emph{carries} the DICOM series, slice
by slice. The reference writer does not emit this --- it references originals --- and a
validator has to accept an issuer that does. It reaches no conformance level, correctly:
\code{UOS-Core} requires a renderable scene and this holds only the volume.\\
\code{series-slice-missing.uos} & \code{error} & A slice is missing, and the validator must name
\emph{which} (\S\ref{sec:volume}).\\
\code{series-slice-altered.uos} & \code{error} & A slice with the same name and size and one byte
changed.\\
\code{series-slice-extra.uos} & \code{error} & A slice the manifest does not declare in
\code{parts[]}. Extra is as serious as missing: nobody knows where it came from.\\
\code{series-slice-deidentified.uos} & \code{warning} & A slice whose tags were cleaned: it
keeps its SOP Instance UID and its pixels and its bytes are no longer the declared ones.
\textbf{This is the case the two levels exist for} --- a validator comparing only file
hashes says ``this series is not this case's'', which is false and is the most expensive
...
\midrule
\endhead
Who maintains it & The editor named on the cover, who holds the copyright. Versions 0.2
and 0.3 were written during the ANFAIA summer grant on a clinical case contributed by
Histora; maintenance is not tied to that term.\\
...
\section{What this version deliberately does not do}\label{sec:limits}
...
Which tooth is this? & \code{derived/seg\_teeth.bin}, per vertex, FDI/ISO-3950. Layer 3
only: a stripped container has no tooth identity, and that is correct.\\
Where is the CBCT? & Not in the container. A \code{kind: volume} directory asset,
\code{external: true}, named \code{sha256:<hex>}, with \code{parts[]} to verify it
slice by slice. What \emph{is} in the ZIP is its \code{.volume.json} sidecar under
\code{volume/}, so you can inspect the geometry with no DICOM parser.\\
...
This document sits across four worlds --- DICOM, FHIR, glTF and medical-device regulation
--- and a competent implementer typically knows one of them. Someone arriving from graphics
knows what a glTF accessor is and has never met a SOP Instance UID; someone arriving from a
PACS is the other way round. Every term below carries a normative rule somewhere in this
document, so it is defined here rather than assumed. The section reference is where the
term does its work.

\begin{longtable}{L{4.3cm} L{11.1cm}}
\toprule
\textbf{Term} & \textbf{Meaning}\\
\midrule
\endhead
\textbf{SaMD} & \emph{Software as a Medical Device}: software whose intended purpose is
medical --- diagnosis, treatment planning --- and which therefore falls under the EU
Medical Device Regulation or the FDA's. The three-layer separation exists so that layers 1
and 2 can be said not to be SaMD and only layer-3 modules are. \S\ref{sec:planes}.\\
\textbf{HIPAA Safe Harbor} & The de-identification method in US health-privacy law that
lists eighteen identifier types to remove: among them every date relative to the individual
except the year, device serial numbers, and full-face photographs \emph{``and comparable
images''}. A three-dimensional reconstruction of a face is a comparable image.
\S\ref{sec:phi}.\\
\textbf{GDPR biometric data} & The EU regulation treats biometric data intended to uniquely
identify a person as a special category, with a reinforced legal basis. Facial geometry and
dentition fall in it. \S\ref{sec:phi}.\\
\textbf{DICOM PS3.15 Annex E} & The part of DICOM that defines \emph{how} to de-identify: a
Basic Application Level Confidentiality Profile plus named \textbf{options} that modify it
--- retain longitudinal temporal information with modified dates, clean descriptors, clean
recognizable visual features, retain device identity, retain UIDs. Declaring the profile
and the options applied is the standard way of saying \emph{what was done} rather than
\emph{how it ended up}. \S\ref{sec:phi}.\\
\textbf{Defacing} & Removing or distorting the facial surface in a volume so it cannot be
reconstructed. It is the practical realisation of PS3.15's \emph{clean recognizable visual
features} option. \S\ref{sec:phi}.\\
\textbf{HMAC} & A keyed hash. Unlike a plain hash it cannot be recomputed without the key,
so an identifier derived through one is not reversible by dictionary --- which a truncated
hash of a clinical identifier is, because the space of identifiers a clinic issues is
small. \S\ref{sec:phi}.\\
\textbf{Frame of Reference UID} & DICOM tag \code{(0020,0052)}. The globally unique
identifier of the coordinate system a series' positions are expressed in. Two series
sharing it are in the same space with no registration needed.
\S\ref{sec:frameid}.\\
\textbf{SOP / Series / Study Instance UID} & DICOM tags \code{(0008,0018)},
\code{(0020,000E)} and \code{(0020,000D)}: the globally unique identifiers of one instance
(a slice), a series and a study. They are what DICOM defines as identity, independently of
file names or header bytes. \S\ref{sec:volume}.\\
\textbf{LPS / RAS} & Anatomical axis conventions. LPS: $+X$ toward the patient's left, $+Y$
posterior, $+Z$ superior --- DICOM's. RAS: $+X$ right, $+Y$ anterior, $+Z$ superior, common
in neuroimaging. Both are right-handed; they differ in the sense of two axes, which is why
handedness alone says nothing. \S\ref{sec:frameid}.\\
\textbf{Hounsfield units (HU)} & The calibrated attenuation scale of conventional CT: water
$=0$, air $=-1000$. CBCT equipment produces grey values that are \emph{not} on that scale
and that vary with the device, the field of view and the position inside the volume.
\S\ref{sec:volume}.\\
\textbf{Spatial Registration Object} & DICOM SOP class
\code{1.2.840.10008.5.1.4.1.1.66.1}, which stores a rigid transform between two Frames of
Reference. It is DICOM's equivalent of an entry in \code{registrations[]}.
\S\ref{sec:mappings}.\\
\textbf{Encapsulated STL Storage} & DICOM SOP class
\code{1.2.840.10008.5.1.4.1.1.104.3} (Supplement 205), which stores an STL inside a PACS
attached to a study. \S\ref{sec:mappings}.\\
\textbf{FHIR \code{Consent}} & The R4 resource recording a patient's consent: scope,
purpose, date, validity. The natural destination of \code{subject.consent}.
\S\ref{sec:consent}.\\
\textbf{FHIR \code{Provenance}} & The R4 resource recording who generated or modified a
resource, when, and from what. It matches a link of \code{provenance/chain.json} field for
field. \S\ref{sec:mappings}.\\
\textbf{FHIR \code{ImagingSelection}} & Introduced in R5 to reference a selection inside an
imaging study --- instances, frames, regions. The destination of
\code{projection.fdi\_targets}, named here so nobody invents a private extension for it.
\S\ref{sec:mappings}.\\
\textbf{SNOMED CT} & The international clinical terminology covering findings, procedures
and anatomical structures, individual teeth included. The expected terminology for
\code{Observation.code} and \code{bodySite} in FHIR. Licensed per country, which is why
this specification ships the structure and leaves the codes unassigned.
\S\ref{sec:vocab}.\\
\textbf{ICDAS} & International Caries Detection and Assessment System: a 0--6 severity
scale per tooth surface, used to code caries comparably between clinicians.
\S\ref{sec:vocab}.\\
\textbf{Universal Numbering System} & The tooth numbering predominant in the US: permanent
1--32, deciduous A--T. It does not agree with FDI/ISO 3950 --- tooth ``27'' is a different
tooth in each. \S\ref{sec:vocab}.\\
\textbf{Bite registration} & The capture of the spatial relation between the upper and
lower arches with the teeth in contact. With an intraoral scanner it comes from an extra
lateral scan, and it is the registration relating the maxilla's frame to the mandible's.
\S\ref{sec:dentistry}.\\
\textbf{Target Registration Error (TRE)} & The error of a registration measured at points
that were \emph{not} used to compute it --- unlike the fit RMS, which is measured on the
points that were and is therefore systematically optimistic. It is the metric that decides
whether a registration serves a given clinical use. \S\ref{sec:dentistry}.\\
\textbf{JWS / COSE Sign1} & Standard detached-signature formats: JWS (JSON Web Signature,
RFC 7515) over JSON, COSE over compact binary. Both admit Ed25519, identify the signing key
by \code{kid}, and allow several signatures over one payload.
\S\ref{sec:provenance}.\\
\bottomrule
\end{longtable}

\newpage
\section{A complete manifest}\label{app:manifest}

Everything above describes the manifest field by field. This is one, whole, so that a
reader who has to write one can see the shape rather than assemble it from fragments.

It is not an illustration: it is the manifest of \code{valid.uos}, the reference container
of the conformance fixture (\S\ref{sec:fixture}), lifted verbatim by the documentation
guardian. The bench is regenerated and validated on every run of the test suite, so this
example is one a validator actually accepts --- and it cannot quietly fall out of date,
because the guardian fails when it does.

It carries three assets, one of them referenced rather than held, a single frame, and no
registrations: a case with one acquisition has nothing to relate. Fields that are
\code{null} are declared and empty, which is not the same as absent --- that distinction is
the one the whole format is built on.

% generado: manifiesto-completo — no editar a mano
\begin{lstlisting}[style=js]
{
 "uos_version": "0.3",
 "case_id": "urn:uuid:f8b5f549-b3a0-5f57-b6cb-dfe16b0bb7f1",
 "created": "2026-09-24T11:46:56.502432Z",
 "generator": {
  "name": "agentic-smart-health",
  "version": "0.14.0"
 },
 "phi_state": "pseudonymized",
 "deidentification": {
  "profile": "DICOM PS3.15 E.1 Basic Application Level Confidentiality Profile",
  "options": [],
  "applied_to": [],
  ...
  "date_shift_days": null,
  "note": "el identificador de paciente se sustituye por un HMAC-SHA256 truncado con sal por emisor (`ASH_PSEUDONYM_SALT`), estable entre adquisiciones y no invertible sin la sal. NO se limpian descriptores ni rasgos visuales reconocibles"
 },
 "purpose_of_use": null,
 "subject": {
  "pseudonym": "FIXTURE-0001",
  "fhir_patient": null,
  "consent": null
 },
 "canonical_frame": {
  ...
  "handedness": "right",
  "anatomical": null,
  "dicom_frame_of_reference_uid": null
 },
 "frames": [
  {
   "id": "frame.ct_001",
   "description": "Volumen CBCT, centrado",
   "units": "mm",
   "handedness": "right",
   "anatomical": null,
   "dicom_frame_of_reference_uid": null
 ...
 "assets": [
  {
   ...
   "kind": "document",
   "visit": "v1",
   "uri": "sha256:af8ea8064a092eb65a578a0085e05b0bd925179072f29c005962604042357d8f",
   "media_type": "model/stl",
   "sha256": "af8ea8064a092eb65a578a0085e05b0bd925179072f29c005962604042357d8f",
   "bytes": 546181,
   "frame": "frame.ios_master",
   "acquisition": {
    "time": "2026-09-24T11:46:56.485807Z",
    "device": {
     "manufacturer": null,
     "model": null,
  ...
  },
  {
   ...
   "sha256": "7c2824dfa844a75975bf74da1929765c5560cab648a5d178dff97aa6e97de505",
   "bytes": 283808,
   "frame": "frame.ios_master",
   "acquisition": {
    "time": null,
  ...
  },
  {
   ...
   "kind": "document",
   "visit": "v1",
   "uri": "clinical/observations.json",
   "media_type": "application/json",
   "sha256": "fd5e4cc86e9387091d4371a2c7cd0b56aa710f3b6d156f6206eb93574409e073",
   "bytes": 2994,
   "frame": "frame.ios_master",
   "acquisition": {
    "time": null,
 ...
 "fhir_map": {
  "case": {
   ...
   "resource": null,
   "note": "el .uos entero como adjunto, content_type application/vnd.histora.uos"
  },
  "asset.ios": {
   "resource_type": "DocumentReference",
   "resource": null,
   "note": "informe u otro documento del caso"
  },
  "asset.scene": {
   "resource_type": "DocumentReference",
   "resource": null,
   "note": "malla y apariencia 3D: FHIR R4 no tiene recurso para geometria dental, y `Media` es para foto, video y audio. `DocumentReference` es el sobre generico de binarios clinicos, que es lo que son"
  },
  "asset.clinical": {
   "resource_type": "Observation",
   "resource": null,
   "note": "una Observation por pieza y por medida, con su `subject` y su `bodySite` en FDI"
  }
 },
 "extensions": {
  "histora_clinical": {
   "name": "histora_clinical",
   "version": "1.0",
   "uri": "clinical/observations.json",
   "schema_id": "histora-clinical/1.0",
   "description": "atributos clinicos por pieza (pH, raices, conductos, hallazgos) y medidas no regionales, con la procedencia de cada valor. El borrador los manda a FHIR (§9) y entonces un .uos suelto no puede contestar que dice el informe de una pieza"
  },
  "histora_reversible": {
   "name": "histora_reversible",
   "version": "1.0",
   "uri": null,
   "schema_id": "histora-reversible/1.0",
   "description": "de `asset.scene` se regenera una malla de arcada con color por vertice, codigo FDI y una columna `medido` que dice que vertices llevan color del paciente y cuales el degradado de respaldo. La geometria y el FDI salen de sus primitivas de malla (`extras.uos_fdi`); el color, de su primitiva `KHR_gaussian_splatting`, tomandolo de las gaussianas que cubren cada vertice dentro de 3 sigmas y ponderandolo por alfa. La malla no viaja: se reconstruye"
  }
 },
 "extensions_used": [
  "histora_clinical",
 ...
 "extensions_required": [],
 "provenance": {
  "prev_manifest_sha256": null,
  "chain": "provenance/chain.json"
 }
}
\end{lstlisting}
% /generado: manifiesto-completo
...
\midrule
\endhead
ZIP entries & 12. ⚠️ Measured on the container as last written, which predates 0.6.0
moving the per-Gaussian FDI labels out of the payloads and into
\code{derived/seg\_gaussians.<layer>.\{bin,meta.json\}}: a container written by today's
writer carries two more entries per labelled layer. The figure is left as measured rather
than recomputed by reading the writer, because this table says \emph{measured}\\
